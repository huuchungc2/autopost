...
GF.imageGen = {
  ...
  async generate(prompt, apiKey, baseUrl) {
    if (GF.aiApi) {
      return GF.aiApi.generateImage(prompt);
    }
  ...
  async saveLocal(base64, filename, settings) {
    if (GF.imageFolder) {
      await GF.imageFolder.saveImage(base64, filename, settings);
      return;
    }
    const s = settings || (GF.storage ? await GF.storage.getSettings() : {});
    if (s.imageSaveLocal === false) return;
    const sub = String(s.imageSaveSubfolder || 'GroupFlow').replace(/^[/\\]+|[/\\]+$/g, '');
    const safeName = String(filename || `groupflow-${Date.now()}.png`).replace(/[/\\]/g, '-');
    const path = sub ? `${sub}/${safeName}` : safeName;
    const blob = this.base64ToBlob(base64);
    const url = URL.createObjectURL(blob);
    await chrome.downloads.download({ url, filename: path, saveAs: s.imageSaveAskEachTime === true });
