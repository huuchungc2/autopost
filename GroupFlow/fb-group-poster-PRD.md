...
├── Generate ảnh → 9Router API (tidien.xyz/v1/images/generations)
├── Load Groups → Scrape cookie session FB (không dùng Graph API)
├── Đăng bài → DOM Automation (1 tab duy nhất)
├── Lưu ảnh → Local (Downloads) + Google Drive (Service Account)
├── Sync metadata → tidien.xyz API (KHÔNG sync ảnh)
├── Comment chéo → Tải bài của mọi user từ tidien, bấm tay hoặc lên lịch
└── Radar Lead → Quét group theo từ khóa, báo lead (local, không sync server)
```
...
| Load groups | Từ FB session: extract toàn bộ group → bộ sưu tập + tìm kiếm (ref: Posting Group Pro). Không nhập tay |
| Bắt post_id | Intercept network request → fallback parse DOM |
...
| Comment chéo | **Bắt buộc** — core feature; bấm tay hoặc lên lịch; tải bài mọi user từ tidien |
| Comment generate | AI trong extension (9Router text model) |
