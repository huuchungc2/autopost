...
**GroupFlow v1.0.271 — Check bài xong panel tự cập nhật số "Của tôi", kết quả check hiện trong Nhật ký (2026-07-16):** Tony báo: "thấy nó bật lên kiểm tra nhưng phần Của tôi không lên, chả hiểu lý do" — đăng bài mới, thấy tab check nền mở lên, nhưng "Của tôi (26)" đứng nguyên. 2 vấn đề độc lập cùng gây ra:

1. **Panel không tự vẽ lại khi check xong**: cron `warmPostAccessCache()` (service worker) ghi kết quả vào `gf_post_access_cache`, nhưng listener `chrome.storage.onChanged` của sidepanel chỉ nghe `extractedGroups`/`activityHistory`/`engineLog`/`postQueue` — cache access đổi thì list Comment, số "Của tôi (N)"/"Đồng đội (N)" và badge KHÔNG cập nhật cho tới khi user tự bấm Làm mới/đổi tab. Bài mới check OK thật sự đã vào danh sách trong storage, chỉ là UI không nhúc nhích. Fix: nghe thêm `changes.gf_post_access_cache` → `schedulePostAccessRefresh()` (debounce 1.5s gom batch nhiều bài check sát nhau thành 1 lần vẽ) → `loadPostedPostsForComment()` — hàm này tự cập nhật badge + số tab con, và chỉ render list khi tab Comment đang mở.
2. **Kết quả check vô hình với user**: check ra "chờ duyệt"/"đã xóa" (bài không vào list là ĐÚNG) chỉ log ra console service worker — user không có chỗ nào thấy lý do. Fix: `_warmPostAccessCacheImpl()` ghi vào Nhật ký (tab Log) mỗi khi trạng thái bài **ĐỔI** so với lần check trước (lần đầu check cũng tính là đổi): "Check bài «snippet»: ✓ Comment được — đã vào danh sách / ⏳ Chờ duyệt — chưa vào / ✕ Đã xóa — loại khỏi danh sách (+ lý do)". Re-check định kỳ ra y kết quả cũ thì im lặng — không spam 26 dòng mỗi 6h. Targets giờ mang kèm `label` (60 ký tự đầu nội dung bài) + `groupName` để dòng log đọc được ngay bài nào/nhóm nào.

Không đụng file trong danh sách bundle của `build-sw-bundle.js` nên không cần rebuild `modules/swBundle.js`.

**Backend + GroupFlow v1.0.270 — Log/Lịch sử hiện tên tác giả cho MỌI entry, kể cả entry cũ và entry đồng bộ từ máy khác (2026-07-16):** Tony gửi ảnh tab Log: entry comment không có tên tác giả dù v1.0.268 đã thêm tag `👤 tên`. 3 nguyên nhân xếp lớp: (1) entry CŨ (ghi trước v1.0.268) không mang field `author_name`/`author_fb_id`; (2) job lên lịch TRƯỚC v1.0.268 mang payload đông lạnh không có field → entry mới sinh từ job cũ vẫn thiếu; (3) **lỗ hổng thật ở đường đồng bộ Log qua server** — bảng `user_activity_log` (migration 037) không có cột tác giả, `pushUnsyncedActivityToServer()` không gửi field, `pullActivityFromServer()` không map → entry kéo về từ máy khác (đa thiết bị) VĨNH VIỄN mất tên tác giả kể cả khi máy gốc có ghi.

Fix 2 phần:
1. **Đường đồng bộ (vá lỗ hổng)**: migration `045_user_activity_log_author.sql` thêm 2 cột `author_name`/`author_fb_id` (+ `ensureUserActivityLogAuthor()` trong `migrationRunner.js`, nối vào chuỗi startup `app.js`); `POST/GET /api/user-sync/activity` (`userSync.js`) ghi/trả 2 field; extension push gửi kèm + pull map lại (`background.js`).
2. **Fallback cho entry cũ (không sửa được dữ liệu đã ghi thì tra ngược)**: `buildActivityAuthorIndex()` (`sidepanel.js`) dựng map `post_id → {tên, fb uid}` từ 3 cache sẵn có (`crossPostsCache` — bài đồng đội kèm `user_name`/`user_fb_id`; `postQueue`/`serverMyPosts` — bài của mình, gắn "Của tôi") mỗi lượt tải Log; render Lịch sử ưu tiên field trên entry, thiếu thì tra map — entry cũ như trong ảnh Tony gửi cũng hiện được tên (bài đã rơi khỏi mọi cache thì đành trống). Tag vẫn là link mở profile FB khi có uid.
...
2. **Tag tác giả trên card Comment**: tag `↔ tên` cũ đổi thành `👤 tên` và — khi có FB uid — thành LINK mở thẳng profile Facebook của tác giả. uid lấy từ field mới `user_fb_id` của `GET /cross-posts` (`up.fb_user_id`, cột có từ migration 039 nhưng **đường sync license-key chưa từng ghi nó** — chỉ route `/group-posts/sync` cũ ghi): vá đủ mạch ghi ở 3 chỗ — `POST /api/user-sync/posts` (`userSync.js`) nhận field optional `fb_user_id` truyền vào `upsertUserPost()` (hàm này đã hỗ trợ sẵn, COALESCE giữ giá trị cũ); `syncLocalPostsToServer()` (`sidepanel.js`) gửi kèm uid actor active (postQueue không lưu actor theo từng bài — sync chạy ngay khi mở panel nên sai lệch hiếm, field chỉ để hiển thị); `reportOwnPendingApproval()` (`background.js`) cũng gửi kèm để nuôi dần uid cho bài ĐÃ có sẵn trên server (đường sync chính chỉ chạy cho bài chưa sync). Bài cũ chưa có uid → tag thường không link, không lỗi.
3. **Bỏ tag "✓ Có thể comment"** (`commentAccessTagHtml()`): từ v1.0.224/236 danh sách CHỈ hiện bài đã xác nhận comment được — tag xanh này thừa 100% (mọi bài đều mang). Bỏ ở cả 2 nguồn; GIỮ tag cảnh báo ⏳ Chờ duyệt/✕ Đã xóa (vẫn cần cho bài đã-comment-xong được giữ lại trong list mà bài gốc sau đó chuyển trạng thái xấu).
4. **Lịch sử ghi rõ bài của AI**: job comment (cả 2 chỗ tạo — `buildRawJobsForOneComment()` + payload `GF_RUN_COMMENT` trong `runComment()`, sidepanel.js) mang thêm `author_name`/`author_fb_id` (đồng đội = tên thật + uid; bài mình = "Của tôi"); `appendHistory()` (4 entry comment trong `runComment()`/`runCommentOwn()`, background.js) ghi 2 field này; render Lịch sử thêm tag `👤 tên` bấm mở FB. Entry cũ không có field → không hiện tag, không lỗi. Job đã lên lịch TRƯỚC bản này (payload đông lạnh không có field) cũng vậy.

**Trả lời "Lâu (20) máy Tèo vs (24) máy Tony"**: KHÔNG phải bug đồng bộ — con số đếm bài "còn trong hạn xác nhận" (`pending_checked_at` trong 6h — `OK_CONFIRMED_TTL_MS` server + `CROSS_POST_CONFIRMED_TTL_MS` client) tại thời điểm MÁY ĐÓ sync/mở tab gần nhất. Máy Lâu tự re-confirm từng bài theo cron (2 bài/3phút) nên tập "bài còn hạn 6h" trôi liên tục; mỗi máy xem giữ snapshot của lần sync của RIÊNG nó (`crossPostsCache` merge cộng dồn + cursor `updated_at` incremental) — máy sync gần đây hơn giữ nhiều bản ghi `pending_checked_at` tươi hơn → đếm được nhiều hơn. 2 máy bấm "Làm mới" cùng lúc sẽ hội tụ (± vài bài đang đúng lúc rơi ra/vào cửa sổ 6h).

**GroupFlow v1.0.267 — Comment timeout 1 lần thì tự "loại bỏ" bài khỏi list/lịch, không đâm đầu chạy lại (2026-07-15):** Tony gửi log 2 lần lỗi `Timeout chờ [aria-label^="Bình luận dưới tên"]...` cho cùng 1 bài (`2885408438373818`, 17:18 và 19:24 cùng ngày) và hỏi "trường hợp này đã fix chưa? ... nếu 1 bài lỗi thì phải có chức năng loại bỏ chứ". Xác nhận: phần "retry vô hạn" ĐÃ fix từ trước (lịch 1 lần lỗi → Bỏ qua + xoá lịch, v1.0.221/231; daily → tối đa 1 lần/ngày, v1.0.261) — 2 dòng log là 2 lượt chạy RIÊNG (2 lịch khác nhau cho cùng bài). Nhưng Tony chỉ đúng 1 lỗ hổng thật: **kết quả comment THẬT bị timeout không được ghi ngược vào `gf_post_access_cache`** — check tab thật (v1.0.258) là fail-open (trang không hiện banner xấu ≠ ô bình luận sẽ xuất hiện, vd nhóm chưa tham gia/khoá bình luận), nên bài vẫn mang trạng thái `'ok'`, vẫn nằm trong list Comment với tag "✓ Có thể comment", và MỌI lịch sau cho đúng bài đó lại mở tab chạy thật rồi timeout y hệt — lặp vô nghĩa đúng như Tony mô tả.

Fix (`background.js`, catch nhánh Cổ điển trong `commentOnPostBgOrClassic()` — đúng chỗ đã set `skippedNotReady` từ v1.0.231): comment thật chính là lần kiểm tra ĐÁNG TIN NHẤT, nên khi timeout chờ ô bình luận thì ghi đè cache access của bài thành `kind: 'pending'` ngay tại đó (`FC.writePostAccessEntry()`). Hệ quả dây chuyền (toàn bộ dùng hạ tầng sẵn có, không thêm cơ chế mới): (1) list Comment ẩn bài + chặn nút Chạy/Lên lịch (`isCommentActionable()`/`splitGroupsByAccess()`, sidepanel.js — chỉ nhận bài cache OK); (2) lịch ĐÃ đặt sẵn cho bài này bị `getPostAccess()` chặn từ sớm ở lượt sau — không mở tab nữa, rơi vào nhánh `noDomFallback` → `skippedNotReady` → "Bỏ qua" êm (chuỗi `reason` cố tình chứa "không thể comment"/"chờ admin" để khớp regex `noDomFallback` — thiếu là rớt xuống Cổ điển mở tab lại, vô hiệu hoá chính fix này); (3) bài CỦA MÌNH (job không mang `crossServerId` — job chéo luôn có) thì `reportOwnPendingApproval()` báo hộ lên server luôn để `/cross-posts` loại bài khỏi list đồng đội ngay. KHÔNG phải loại bỏ vĩnh viễn: `'pending'` hết hạn sau `PENDING_ACCESS_TTL_MS` (20 phút) — cron `warmPostAccessCache()` tự check lại nền bằng tab thật, bài được admin duyệt/đã vào nhóm thì tự quay lại list, đúng semantics "coi như một lượt, để hệ thống tự check lại sau" thay vì comment mù.

Không đụng file trong danh sách bundle của `build-sw-bundle.js` (chỉ `background.js`) nên không cần rebuild `modules/swBundle.js`.

**GroupFlow v1.0.266 — "Tắt hết lịch vẫn chạy": vá nốt 3 đường hủy lịch còn sót sau v1.0.261 (2026-07-15):** Tony báo lại đúng hiện tượng cũ — đã tắt/hủy hết lịch mà bài vẫn tự đăng. Rà lại toàn bộ các đường "bỏ lịch" thì thấy fix v1.0.261 (xoá `ngay_dang`/`gio_dang` khi hủy) mới chỉ áp cho đường hủy HÀNG LOẠT (`cancelSelectedPostSchedules()`, `sidepanel.js`) — còn sót 3 đường khác, mỗi đường một kiểu để lại "tàn dư" đủ cho lịch tự sống lại:

1. **Hủy TỪNG lịch ở tab Hoạt động (`cancelUpcoming()`, `sidepanel.js`)** — chỉ xoá entry `activityUpcoming` + alarm, GIỮ NGUYÊN `ngay_dang`/`gio_dang` trên bài trong `postQueue`. `reconcileQueueSchedules()` (`background.js` — chạy mỗi phút qua `gf_retry_missed` + mỗi lần service worker khởi động) thấy "bài có giờ hẹn nhưng không có lịch khớp" rồi tự dựng lại lịch trong ≤1 phút; tệ nhất là khi giờ hẹn đã QUA (hủy sau giờ hẹn, hoặc hủy lúc job đang chờ trong hàng đợi): nhánh `when <= now` của reconcile gọi thẳng `runScheduledJob()` **không kèm `alarmName`** — tức bỏ qua luôn cả bước re-check hủy `isUpcomingStillActive()` vừa thêm ở v1.0.261 — bài đăng NGAY, không gì cản được. Fix 2 phía: (a) `cancelUpcoming()` giờ đi qua `cancelPostScheduleAlarms(postId)` (nâng cấp từ hàm mồ côi cùng tên — vốn không được ai gọi và cũng không persist kết quả) dọn đủ cả 3 dấu vết: alarm + payload `alarm_<tên>`, entry `activityUpcoming` (kể cả `generate_image` gắn theo bài), và `ngay_dang`/`gio_dang` trên bài; (b) nhánh `when <= now` của `reconcileQueueSchedules()` đổi từ chạy thẳng sang `registerPostScheduleFromQueue(post, now)` — job chạy bù giờ đi ĐÚNG đường lịch chuẩn (entry + alarm thật, luôn có `alarmName`) nên mọi lần chạy đều qua re-check hủy; vẫn chạy bù ngay trong cùng tick vì `retryMissedActivity()` await reconcile xong mới đọc `activityUpcoming` nên nhặt được entry `when<=now` vừa tạo luôn ở vòng lặp bên đó.
2. **Đổi/xoá giờ hẹn trong modal sửa bài (`saveComposePostToQueue()`/`applyComposerToQueuePost()`, `sidepanel.js`)** — trước đây CHỈ ghi 2 field mới lên bài: alarm + entry cũ vẫn giữ nguyên GIỜ CŨ với payload đông lạnh, tới giờ cũ alarm vẫn bắn — và re-check `isUpcomingStillActive()` lại thấy entry còn nguyên (chưa ai dọn) nên bài vẫn đăng theo lịch user tưởng đã bỏ. Fix: sau khi lưu, nếu giờ hẹn THAY ĐỔI thì dọn lịch "1 lần" cũ (`cancelPostScheduleAlarms(post.id, { clearQueueFields: false })` — 2 field vừa mang giá trị mới, không được xoá); nếu đặt giờ mới thì bắn `GF_RECONCILE_SCHEDULES` để lịch mới đăng ký liền, không đợi tick 1 phút.
3. **Xoá bài khỏi hàng đợi (`data-del-post` + `bulkDeletePosts()`, `sidepanel.js`)** — không dọn gì cả: alarm cũ vẫn bắn đúng giờ và `refreshScheduledPostPayload()` (`background.js`) khi không tìm thấy bài trong queue thì **fallback về stub payload đông lạnh** — bài ĐÃ XOÁ vẫn được đăng nguyên văn; lịch "lặp lại hàng ngày" còn tệ hơn (payload entry tự chứa nguyên bài, không cần queue). Fix: cả 2 đường xoá đều gọi `cancelPostScheduleAlarms(id, { clearQueueFields: false })` + `removeDailySchedulesForPost(id)` (helper mới — xoá entry `dailyFixedSchedules` kind `post` có `payload.posts[0].id` khớp bài bị xoá).

Nhân tiện vá luôn 1 lỗi lệch dữ liệu cùng họ ở `rescheduleUpcoming()` ("Sửa giờ" ở tab Hoạt động): trước đây đổi giờ chỉ ghi entry/alarm mới, `ngay_dang`/`gio_dang` trên bài vẫn giữ GIỜ CŨ (tag trên card hiện sai giờ) — giờ đồng bộ giờ mới ngược vào bài (bắt buộc phải làm sau khi `cancelUpcoming()` bên trong nó đã xoá trắng 2 field).

Không đụng file nào trong danh sách bundle của `build-sw-bundle.js` (chỉ `background.js` + `sidepanel.js`) nên không cần rebuild `modules/swBundle.js`.

**Backend + GroupFlow v1.0.265 — Client tự lọc/ngừng check bài quá N ngày đã lỡ nằm sẵn trong cache cục bộ (2026-07-13):** Ngay sau khi chặn server không trả bài quá N ngày (mục ngay dưới), Tony hỏi tiếp: "bài cần kiểm tra có phải kiểm tra nữa không vì kiểm tra cũng vô nghĩa" — chỉ ra đúng 1 lỗ hổng: chặn ở server chỉ ngăn được bài MỚI không tải về nữa, nhưng bài ĐÃ LỠ tải về từ trước (nằm sẵn trong `postQueue`/`serverMyPosts`/`crossPostsCache`, `chrome.storage.local`) thì không hề bị dọn — các cache này đều là kiểu MERGE CỘNG DỒN (`mergeUserPostsById()` và tương tự), không bao giờ tự xoá bớt entry cũ chỉ vì server ngừng trả nó nữa. Hệ quả: bài quá hạn N ngày ĐÃ CÓ SẴN trong cache vẫn tiếp tục (1) hiện ra trong tab Comment và (2) bị `warmPostAccessCache()` (cron 3 phút) tự mở tab kiểm tra "còn comment được không" — vô nghĩa vì chính sách đã coi bài đó là hết hạn.

Tony chốt 2 quyết định rõ ràng: (1) bài quá hạn nên **ẩn hẳn** khỏi cả 2 tab "Của tôi"/"Đồng đội" (không chỉ ngừng check ngầm mà vẫn hiện), và (2) áp luật N ngày **đồng nhất** cho MỌI nguồn bài, kể cả `postQueue` (bài tự tạo qua extension, không lấy từ server — trước giờ không có giới hạn tuổi nào).

Fix gồm 3 phần:
1. **Backend**: thêm `GET /api/user-sync/config` (`userSync.js`, `authenticateLicenseKey`) — endpoint nhẹ trả `{ posts_sync_lookback_days }`, để extension tự đọc lại đúng N đang cấu hình trên website (không có route này thì extension không có cách nào biết N hiện tại, vì `GET /api/settings` dùng JWT auth của website, extension chỉ có license key).
2. **`background.js`**: thêm `getPostsSyncLookbackDays()` (gọi `/config`, cache 1 giờ/lần trong `chrome.storage.local` — giá trị hiếm khi đổi tay, fail-open về giá trị cache cũ hoặc mặc định 60 nếu mất mạng, không chặn hẳn việc check chỉ vì lỗi mạng) + `isWithinPostsSyncLookback(dateStr, days)` (coi CÒN hạn nếu thiếu field ngày — an toàn hơn lỡ ẩn nhầm). Áp vào `_warmPostAccessCacheImpl()` — cả `d.postQueue` (theo `p.lastPostedAt`) lẫn `d.serverMyPosts` (theo `sp.posted_at`) đều bị lọc TRƯỚC khi đưa vào `targets`, nên bài quá hạn không còn được thêm vào hàng đợi tự check nữa (lưu ý: hàm này vốn KHÔNG quét `crossPostsCache`, xem ghi chú v1.0.236 — check bài người khác luôn fail-open, không đáng làm).
3. **`sidepanel.js`**: bản sao độc lập của 2 hàm trên (2 context khác nhau — service worker vs sidepanel, cùng pattern mirror đã dùng cho `modules/scheduler.js`). Áp vào `loadPostedPostsForComment()` — lọc cả 3 nguồn (`localPosts` theo `p.lastPostedAt`, `myServerItems` theo `sp.posted_at`, `crossItems` theo `cp.posted_at`) TRƯỚC khi gộp vào `state.comments` — bài quá hạn biến mất hoàn toàn khỏi cả 2 tab, đúng quyết định "ẩn hẳn" của Tony.

Chỉ lọc ở thời điểm ĐỌC (không xoá gì khỏi `chrome.storage.local`) — nếu Tony sau này tăng N lớn hơn, bài cũ tự động hiện lại ngay từ cache đã có sẵn, không cần đồng bộ lại từ server.

**Backend + Frontend — Giới hạn số ngày đồng bộ bài viết GroupFlow, cấu hình từ website (2026-07-13):** Sau khi sửa xong vụ "24 vs 17" (mục ngay dưới đây), Tony đặt câu hỏi tiếp theo: muốn thêm cấu hình "bài viết trả về N ngày (mặc định 60) kể từ ngày post, chứ không trả về nhiều quá server quá tải" — và nhấn mạnh rõ: cấu hình này phải nằm ở **website** (không phải extension), vì mục đích là chặn TỪ GỐC phía server, không tải về/không check những bài quá cũ nữa.

Trước bản này: `GET /api/user-sync/cross-posts` (`userSync.js`) có `CROSS_POSTS_LOOKBACK_MS` hardcode cứng 30 ngày (không cấu hình được, và lọc theo `updated_at` chứ không đúng đúng nghĩa "kể từ ngày post"); `GET /api/user-sync/my-posts` (bài của chính mình) **không hề có giới hạn tuổi bài nào cả** — về lý thuyết trả về toàn bộ lịch sử `user_posts` của 1 tài khoản nếu cold-start (`since` rỗng) chưa đủ `limit` cắt bớt.

Thêm setting `posts_sync_lookback_days` vào `app_settings` (`appSettingsService.js`) — `getEffectivePostsSyncLookbackDays()` đọc cache trong bộ nhớ (mặc định `60` nếu chưa cấu hình, theo đúng số Tony chọn), `savePostsSyncLookbackDays(days)` validate số nguyên dương rồi ghi `INSERT ... ON DUPLICATE KEY UPDATE`. Áp dụng field này ở CẢ 2 route (`userSync.js`, helper dùng chung `getPostsSyncLookbackFloor()`):
- `GET /my-posts`: thêm `AND posted_at > ?` (lookbackFloor) vào WHERE của CẢ 2 nhánh (cold-start lẫn `since` incremental) — route này trước đây hoàn toàn không có bound này.
- `GET /cross-posts`: thêm `AND up.posted_at > ?` vào `baseWhere` (dùng chung cho cả 2 nhánh) — SONG SONG với bound `updated_at` cũ (giữ nguyên, vẫn cần cho tối ưu index/cursor logic, xem comment gốc) chứ không thay thế, vì 2 bound phục vụ 2 mục đích khác nhau: `updated_at` là tối ưu hiệu năng + cursor đồng bộ, còn `posted_at` mới là cắt tuổi bài THẬT theo đúng yêu cầu "kể từ ngày post".

Route mới `PUT /api/settings/posts-sync-lookback` (`settings.js`, `requireRole('super_admin')`) — nhận `{ days }`, gọi `savePostsSyncLookbackDays()`. `GET /api/settings` (route tổng quan có sẵn) thêm field `config.posts_sync_lookback_days` để frontend đọc giá trị hiện tại. UI: `GroupExtensionSettings.jsx` (component hiện API key/license key cá nhân, mount trong tab "Extension" của trang Cài đặt) thêm 1 block MỚI — ô nhập số + nút "Lưu" — CHỈ hiện khi `user.role === 'super_admin'` (đây là cấu hình toàn hệ thống, không phải cấu hình cá nhân như phần API key/license phía trên cùng component).

Vì bài không được sync về (`/my-posts`/`/cross-posts` không trả nữa) thì tự nhiên không còn nằm trong danh sách extension dùng để quyết định bài nào cần tự check "còn comment được không" (`warmPostAccessCache()`, quét `postQueue`/`serverMyPosts`/`crossPostsCache` — toàn bộ đều bắt nguồn từ 2 route này) — không cần sửa gì thêm ở phía "check", chặn ngay từ nguồn cấp dữ liệu là đủ.

**Backend + GroupFlow v1.0.264 — Bài đồng đội đã comment bị ẩn khỏi tổng số, thêm thống kê "Của tôi"/"Đồng đội" (2026-07-13):** Tony cài lên 3 máy thật (Tèo — `ngohuuchung9@gmail.com`, Tony/supperadmin, Trưởng — `lau@gmail.com`) để test tính năng comment chéo, phát hiện số bài hiện ra cho CÙNG 1 người (Lâu) khác nhau giữa 2 máy đang xem: máy Tèo thấy "Lâu (24)", máy Tony thấy "Lâu (17)" — tưởng là lỗi đồng bộ ("không hiểu quy tắc đồng bộ kiểu sao").

Root cause: `GET /api/user-sync/cross-posts` (`userSync.js`) từ thiết kế opt-in v1.0.236/v1.0.246 có điều kiện `AND NOT EXISTS (SELECT 1 FROM user_post_comments upc WHERE upc.user_post_id = up.id AND upc.commenter_user_id = ?)` trong WHERE — loại hẳn khỏi kết quả trả về bất kỳ bài nào mà **CHÍNH NGƯỜI ĐANG GỬI REQUEST** đã từng tự comment rồi. Vì mỗi người tự comment 1 số lượng bài khác nhau của Lâu từ trước, tổng số bài "còn thấy được" của Lâu tất yếu khác nhau tuỳ người xem — đây không phải lỗi đồng bộ, mà là hệ quả trực tiếp của điều kiện lọc theo TỪNG NGƯỜI XEM. Field `needs_comment` trong SELECT cũng bị hardcode cứng `1 AS needs_comment` (không tính thật) — hoá ra đây là tàn tích của 1 thiết kế ĐÃ TỪNG tồn tại trước đó: phía client (`isCommentDone()`, `sidepanel.js`: `if (c._source === 'cross') return c._needsComment === false;`) đã có sẵn đầy đủ hạ tầng đọc field `needs_comment` để đánh dấu tag "✓ Đã comment" MÀ KHÔNG lọc bài đó mất khỏi danh sách/tổng số (kèm luôn nút xác nhận "Bài này đã comment rồi — vẫn chạy để đẩy bài thêm 1 lần nữa?" ở `renderComments()`) — nhưng backend đã đổi hướng sang loại-trừ-qua-WHERE (opt-in redesign) mà quên bỏ luôn phần hardcode `needs_comment`, khiến hạ tầng client có sẵn này chưa bao giờ thực sự phát huy tác dụng.

Tony chốt rõ mục tiêu đúng: "tao có X bài kiểm tra được phép comment thì luôn hiển thị X ở đồng đội của người khác" — tức muốn thấy ĐỦ toàn bộ số bài đã duyệt của đồng đội (phục vụ mục đích "đẩy bài" — có thể muốn chạy lại nhiều lần cho cùng 1 bài), không phải 1 danh sách việc-cần-làm co dần theo lịch sử riêng của từng người xem.

Fix: bỏ hẳn `NOT EXISTS (...)` khỏi WHERE của `/cross-posts` — mọi bài đã duyệt (`pending_approval=0` + `pending_checked_at` còn hạn 6h + `visible_after` đã qua) của MỌI người khác đều luôn trả về, không phụ thuộc người xem đã tự comment hay chưa. Đổi `needs_comment` từ hardcode sang tính THẬT bằng correlated subquery theo đúng `myId` của người đang hỏi — không cần sửa gì thêm phía client, hạ tầng tag "✓ Đã comment"/nút "đẩy bài thêm lần nữa" đã sẵn sàng dùng ngay. Thêm thống kê tổng số ngay trên 2 nút tab con "Của tôi (N)"/"Đồng đội (M)" (`updateCommentSubTabCounts()`, `sidepanel.js` + `sidepanel.html`) — cùng điều kiện `isCommentActionable()` với danh sách thật, giúp so sánh nhanh giữa nhiều máy mà không cần đếm tay từng dòng.

Lưu ý: hiện tượng RIÊNG của máy Trưởng (hoàn toàn không thấy bài đồng đội nào, "0" tuyệt đối chứ không phải thấy ít hơn) là 1 vấn đề KHÁC — nghi vấn cao nhất là license key/đồng bộ hỏng ở đúng máy đó, chưa xác nhận được nguyên nhân cụ thể (cần Tony tự kiểm tra license key ở tab Cài đặt hoặc Console trên đúng máy Trưởng), không liên quan tới bug `NOT EXISTS` vừa sửa ở trên.

**GroupFlow v1.0.263 — 2 bug phát hiện qua code review trước release v1.0.261/262 (2026-07-13):** Tony yêu cầu review lại code trước khi release. Chạy `/code-review` (high effort) trên diff của v1.0.261 (fix "hủy lịch vẫn chạy") + v1.0.262 (random hoá khoảng cách) — 2 finder agent độc lập cùng phát hiện 2 bug thật, cả 2 nằm trong `background.js`:

1. **`retryMissedActivity()` có thể "hồi sinh" lại đúng lịch vừa bị hủy, vô hiệu hoá fix v1.0.261.** Hàm này đọc `activityUpcoming` MỘT LẦN vào đầu hàm (`const d = await chrome.storage.local.get(...)`), tách thành `due` (tới hạn) và `remaining` (chưa tới hạn), rồi xử lý TUẦN TỰ từng item `due` — mỗi item đi qua `await this.runScheduledJob(...)`, có thể mất vài giây tới vài phút nếu hàng đợi `_taskQueue` đang bận việc khác. Ở CUỐI hàm, `await chrome.storage.local.set({ activityUpcoming: remaining })` ghi đè NGUYÊN CỤM `activityUpcoming` bằng snapshot `remaining` đã tính TỪ ĐẦU hàm. Nếu user bấm "Hủy lịch" cho 1 item KHÁC (chưa tới hạn, đang nằm trong `remaining`) hoặc tự đặt thêm 1 lịch mới NGAY TRONG LÚC hàm này còn đang await xử lý dở các item due — thay đổi đó (đã ghi thành công vào storage bởi `cancelUpcoming()`) bị GHI ĐÈ MẤT bởi lệnh `set()` cuối hàm dùng snapshot cũ — lịch vừa hủy bị "hồi sinh" lại y hệt trước khi hủy. Bug này không do diff v1.0.261 tạo ra (code này không đổi gì) nhưng trực tiếp làm vô hiệu hoá đúng cơ chế re-check-trước-khi-chạy (`isUpcomingStillActive()`) vừa thêm ở bản đó — vì check đó tin vào `activityUpcoming`, mà chính storage đó có thể bị ghi đè sai bởi race này. Fix: bỏ hẳn việc tự ghi lại `activityUpcoming` ở cuối `retryMissedActivity()` — hoá ra hoàn toàn KHÔNG CẦN: item chạy THÀNH CÔNG (`ok === true`) đã được `removeUpcomingByAlarmName()` (gọi bên trong `runScheduledJob()`) tự dọn bằng cách đọc-lọc-ghi storage MỚI NHẤT tại đúng thời điểm đó rồi; item lỗi/chưa tới hạn thì vốn dĩ không ai đụng vào, cứ để nguyên trong storage. Đơn giản hoá luôn: bỏ biến `remaining` không dùng nữa, `due` tính thẳng bằng `.filter()`.
2. **Thông báo "Bắt đầu đăng bài…"/"Bắt đầu comment…" vẫn hiện dù job đã bị hủy.** `runScheduledJob()` bắn `chrome.notifications.create(...)` NGAY khi vào hàm — TRƯỚC CẢ bước re-check hủy (`isUpcomingStillActive()`) nằm bên trong closure đã `enqueueTask()`. Nếu job bị hủy đúng lúc đang chờ tới lượt trong hàng đợi, user vẫn thấy thông báo "Bắt đầu…" hiện ra dù job sẽ bị bỏ qua ngay sau đó — gây hiểu lầm gần giống hệt bug gốc đang cố sửa (khác biệt duy nhất là lần này không thật sự đăng lên Facebook, nhưng người dùng nhìn thông báo vẫn tưởng có chạy). Fix: tách thông báo ra thành `showScheduleNotice()` (closure nhỏ, dùng chung), gọi nó ở ĐÚNG bên trong closure của `enqueueTask()`, ngay SAU khi xác nhận `isUpcomingStillActive()` trả `true` — post/comment giờ chỉ hiện thông báo khi job THẬT SỰ sắp chạy. `generate_image` và nhánh mặc định (không có bước re-check hủy nào) vẫn bắn thông báo ngay như cũ, không đổi.

Nhân tiện áp luôn 1 gợi ý dọn code từ review: `computeStaggeredWhens()` (`sidepanel.js`, thêm ở v1.0.262) đổi từ tự viết công thức `gapMs * (0.7 + Math.random() * 0.6)` sang gọi `GF.scheduler.randBetween([Math.round(gapMs*0.7), Math.round(gapMs*1.3)])` — helper random có sẵn (đã load cho sidepanel qua `modules/scheduler.js`, cùng hàm `scheduleCommentJobsOnce()` đang dùng cho `betweenComments`) thay vì có 2 kiểu random khác nhau trong cùng 1 file.

Không đụng file nào nằm trong danh sách bundle của `build-sw-bundle.js` nên không cần rebuild `modules/swBundle.js`.

**GroupFlow v1.0.262 — Random hoá khoảng cách đăng bài/comment, chống dấu hiệu "chạy đều như máy" (2026-07-13):** Tony chỉ ra: khoảng cách CỐ ĐỊNH giữa các bài (đúng 15 phút/bài, đúng 1 tiếng/bài, hoặc lịch "lặp lại hàng ngày" chạy đúng y 1 giờ mỗi ngày) là 1 kiểu vân tay hành vi máy dễ bị Facebook nhận ra — người thật không bao giờ đăng cách nhau đúng y 1 khoảng thời gian lặp lại nhiều lần. Có 3 chỗ trong code đang tạo ra sự đều đặn này, cả 3 đều được random hoá trong bản này:

1. **"Lên lịch đã chọn" (bulk, cả Tạo bài lẫn Comment)** — `confirmCampaignStagger()` và `scheduleSelectedComments()` (`sidepanel.js`) trước đây tính mốc giờ cho bài thứ `i` bằng công thức TUYỆT ĐỐI `startWhen + i * gapMs` (`gapMs` = giá trị+đơn vị user tự nhập, vd "15 phút") — khoảng cách giữa MỌI cặp bài liên tiếp luôn giống hệt nhau tới từng giây. Thêm `computeStaggeredWhens(startWhen, gapMs, count)` — thay vì random tuyệt đối theo index (dễ làm đảo thứ tự 2 bài nếu jitter lớn hơn nửa gap), hàm này random hoá KHOẢNG CÁCH giữa mỗi cặp liên tiếp: mỗi bước nhân `gapMs` với 1 hệ số ngẫu nhiên 0.7–1.3 lần rồi CỘNG DỒN từ mốc trước đó (không phải từ mốc bắt đầu) — giữ đúng tốc độ đăng TRUNG BÌNH mà user chọn (vd trung bình vẫn ~15 phút/bài) nhưng không còn 2 khoảng cách nào giống hệt nhau. Mốc bài ĐẦU TIÊN (index 0) cố tình giữ nguyên đúng giờ user chọn, không jitter — vì đó là lựa chọn tường minh (giờ bắt đầu), không phải khoảng cách lặp lại. `gapMs = 0` (user chủ đích muốn đăng dồn cùng lúc, "Lên lịch" không tick giãn cách) thì giữ nguyên 0, không tự thêm giãn cách ngoài ý muốn.
2. **`betweenPosts` (giãn cách các job chạy qua hàng đợi/`runSpacedJob()`, và giữa các bài trong 1 batch `runPostMatrix()`)** — trước đây là 1 SỐ CỐ ĐỊNH theo `securityLevel` (fast=180s, balanced=420s, safe=900s) chỉ được cộng thêm 0-60 giây ngẫu nhiên LÚC DÙNG (`randBetween([betweenPosts, betweenPosts+60])`) — biên độ dao động quá hẹp so với giá trị gốc (vd "safe" luôn ra đúng 900-960 giây, tức 15-16 phút, MỌI LẦN). Đổi `getSecurityDelays()` (`background.js`, và bản mirror `modules/scheduler.js.DELAYS` dùng cho sidepanel) từ số cố định sang RANGE thật, cùng dạng với `betweenGroups`/`betweenComments` đã có sẵn: fast `[120,300]`, balanced `[300,600]`, safe `[600,1200]` (giây) — biên độ tương đương cũ về tốc độ trung bình nhưng đủ rộng để không lộ chu kỳ cố định. 3 nơi gọi (`runSpacedJob()`, `tickDailyFixedSchedules()`, `runPostMatrix()`) đơn giản hoá từ `randBetween([betweenPosts, betweenPosts+60])` thành `randBetween(betweenPosts)` (đã là range).
3. **Giờ chạy thật của lịch "lặp lại hàng ngày"** — `tickDailyFixedSchedules()` (`background.js`) trước đây chỉ jitter 0-20 GIÂY cho entry đầu tiên tới giờ trong mỗi tick — nghĩa là lịch "chạy lúc 8:00 sáng mỗi ngày" gần như luôn bắn đúng trong vòng 20 giây quanh 8:00:00, đúng y hệt mỗi ngày — dấu hiệu cron cố định rất rõ. Thêm `dailyJitter` vào map `getSecurityDelays()` (0 tới 5/10/15 phút theo fast/balanced/safe) — dùng thay cho hằng số 20 giây cũ. Quan trọng: chỉ jitter TRỄ HƠN (0 tới +N phút), không bao giờ SỚM HƠN giờ user đặt — vì tick chỉ phát hiện + claim entry SAU KHI đã tới giờ thật (`entry.timeOfDay > hhmm` mới `continue`, tức bỏ qua nếu CHƯA tới giờ), nên về mặt kỹ thuật không thể chạy sớm hơn mà không đổi lại điều kiện này; giữ delay-only cũng tránh gây bất ngờ kiểu "chưa tới giờ đã chạy" nếu user đang theo dõi trực tiếp.

Không đụng file nào nằm trong danh sách bundle của `build-sw-bundle.js` (chỉ `background.js`, `sidepanel.js`, `modules/scheduler.js` — 2 file sau chỉ load qua `<script>` cho `sidepanel.html`, không phải service worker) nên không cần rebuild `modules/swBundle.js`.

**GroupFlow v1.0.261 — Hủy lịch rồi vẫn tự chạy: race "job đã enqueue" + daily schedule retry vô hạn khi lỗi (2026-07-13):** Tony báo hủy lịch xong bài/comment vẫn tự chạy — gửi kèm ảnh cụ thể 1 bài trong nhóm CHƯA THAM GIA (Facebook trả "This content isn't available right now", không hề có ô bình luận) nhưng "Cổ điển" cứ tự mở lại chạy timeout liên tục nhiều lần. 2 root cause độc lập, cả 2 nằm trong `background.js`:

1. **Race: hủy không rút lại được job đã enqueue.** Lịch (cả "1 lần cụ thể" lẫn "lặp lại hàng ngày") được "nhận" (claim) và đưa vào `_taskQueue` NGAY khi tick/alarm phát hiện tới giờ — nhưng thực thi thật (`runPostMatrix()`/`runComment()`) có thể trễ vài phút sau đó (giãn cách `betweenPosts`/`betweenComments` của `runSpacedJob()`, hoặc hàng đợi đang bận việc tay/việc khác). Trong lúc trễ này, tag lịch trên UI vẫn hiện bình thường (bản ghi `activityUpcoming`/`dailyFixedSchedules` còn nguyên, chỉ thêm cờ `pendingRunDate`/đã claim) — user bấm "Hủy lịch" thấy tag biến mất tưởng đã chặn được, nhưng `cancelUpcoming()`/`cancelDailyFixedSchedule()` (`sidepanel.js`) chỉ xoá bản ghi khỏi `chrome.storage.local` — **không có cách nào rút lại 1 closure JS đã nằm sẵn trong `_taskQueue`** (bản chất Promise chain, không có API "cancel" cho task đã enqueue). Fix: `runScheduledJob()` (đường lịch 1 lần, cả từ alarm thật lẫn `retryMissedActivity()`) và `tickDailyFixedSchedules()` (đường lặp lại hàng ngày) giờ RE-CHECK storage ngay TRƯỚC KHI chạy thật (bên trong closure đã enqueue, sau khi đã qua `delay(gapMs)`/xếp hàng) — nếu bản ghi đã bị xoá (hủy) thì bỏ qua êm, không chạy. Thêm helper `isUpcomingStillActive(alarmName)` (đặt cạnh `removeUpcomingByAlarmName()`) — đọc lại `activityUpcoming`, coi còn tồn tại (theo `alarmName`/`id`) là tín hiệu "vẫn còn muốn chạy".

2. **Lịch lặp lại hàng ngày tự retry vô hạn khi job THẤT BẠI (nguyên nhân chính gây "chạy hoài").** `tickDailyFixedSchedules()` trước đây chỉ gọi `markDailyScheduleDone()` (set `lastRunDate = today`) ở nhánh THÀNH CÔNG của khối `try` — mọi lỗi ném ra (kể cả lỗi thật sự VĨNH VIỄN như bài không xem được vì chưa tham gia nhóm, `waitFor()` timeout không bao giờ hết) đều rơi vào `catch` chỉ log cảnh báo, không set `lastRunDate`. `pendingRunDate` của entry đó vẫn còn "hôm nay" nên các tick TIẾP THEO trong cùng phiên service worker tự bỏ qua đúng như thiết kế — nhưng `recoverStalledDailySchedules()` (chỉ gọi lúc `chrome.runtime.onStartup`, tức là MỖI LẦN service worker khởi động lại — xảy ra RẤT thường xuyên dưới Manifest V3 do idle timeout, đóng/mở lại Chrome, reload extension...) có giả định SAI: coi MỌI entry có `pendingRunDate !== lastRunDate` là "job phiên trước bị NGẮT GIỮA CHỪNG" (crash/tắt máy) rồi tự xoá `pendingRunDate` để chạy bù — giả định này đúng cho trường hợp bị ngắt thật, nhưng SAI cho trường hợp job đã CHẠY XONG HOÀN CHỈNH nhưng kết quả là LỖI (không phải bị ngắt). Hệ quả: bài lỗi cố định (như case Tony gặp) bị chạy lại từ đầu mỗi khi service worker restart — có thể hàng chục lần/ngày, vô thời hạn, dù dùng "Hủy lịch" cũng không giúp gì nếu entry đó vẫn còn tồn tại trong danh sách (chưa bị xoá hẳn). Fix: luôn gọi `markDailyScheduleDone()` sau khi thử chạy — dù thành công hay thất bại (đưa ra khỏi khối `try`, chạy vô điều kiện) — khôi phục đúng thiết kế gốc "tối đa 1 lần/ngày cho mỗi entry"; lỗi thật (network tạm thời, hay lỗi vĩnh viễn như case này) đều tự thử lại đúng 1 lần vào NGÀY MAI, không còn dồn dập trong cùng 1 ngày. Lưu ý: nếu lỗi là VĨNH VIỄN (như chưa tham gia nhóm), vẫn sẽ thử lại mỗi ngày 1 lần cho tới khi Tony chủ động hủy hẳn lịch đó hoặc tham gia nhóm — đây là hành vi ĐÚNG theo thiết kế "lặp lại hàng ngày" (không phải bug), khác với bug "chạy nhiều lần/ngày" đã sửa ở đây.

Cả 2 fix chỉ đụng `background.js` (không đụng file nào trong danh sách bundle của `build-sw-bundle.js`) nên không cần rebuild `modules/swBundle.js`.

**GroupFlow v1.0.260 — Lịch comment tới giờ không chạy (deadlock) + thông báo sai "Đăng theo lịch" cho lịch comment (2026-07-12):** Tony báo 2 hiện tượng sau khi lên v1.0.258/259 (check bằng tab thật): (1) đặt lịch comment "1 lần cụ thể", tới giờ chỉ thấy 1 thông báo "Bắt đầu…" hiện ra rồi im luôn — comment thật không bao giờ lên Facebook; (2) thông báo đó ghi tiêu đề "GroupFlow — Đăng theo lịch" / "Bắt đầu đăng bài…" dù đây là lịch COMMENT, không có bài đăng nào cả.

Root cause hiện tượng (1) — **deadlock thật**, hệ quả trực tiếp của chính thay đổi v1.0.258: `GF_BG._taskQueue` là 1 chuỗi Promise tuần tự DUY NHẤT dùng chung cho MỌI thao tác đăng bài/comment (`enqueueTask()`) — tại 1 thời điểm chỉ đúng 1 task được chạy, task sau phải đợi task trước resolve xong mới tới lượt. Đường lịch comment thật đi qua: `runScheduledJob()` → `enqueueTask(() => runSpacedJob('comment', () => runComment(payload)))` → `runComment()` → `commentOnPostBgOrClassic()` → `FC.commentOnPost()` (`fbCommentBg.js`) → `getPostAccess()` → (cache miss/stale) → `GF_BG.checkPostCommentableViaTab()` — và hàm này (thêm ở v1.0.258) lại tự bọc chính nó trong **`this.enqueueTask()` LẦN NỮA**. Vì đang gọi từ BÊN TRONG 1 task đã chiếm chỗ của hàng đợi (task ngoài — chính `runScheduledJob`), lệnh `enqueueTask()` lồng này bị xếp vào NGAY SAU task ngoài trong cùng `_taskQueue` — nhưng task ngoài lại đang `await` đúng lệnh enqueue lồng đó để tiếp tục chạy → task ngoài không bao giờ resolve (nên task trong không bao giờ tới lượt) và task trong không bao giờ chạy (nên task ngoài không bao giờ resolve) → **kẹt cứng vĩnh viễn, không throw lỗi gì** (khác hẳn 1 lỗi thật sẽ rơi vào `catch` của `runScheduledJob()` và hiện thông báo "Lịch lỗi"). Thông báo "Bắt đầu…" đã bắn ra TRƯỚC khi chạm chỗ kẹt (nằm ngay đầu `runScheduledJob()`, trước khi vào `enqueueTask`) nên trông như job đã bắt đầu chạy — nhưng thực chất treo vô thời hạn tới khi Chrome tự huỷ service worker (idle timeout của Manifest V3), lúc đó promise bị bỏ dở hoàn toàn, không còn dấu vết gì để chẩn đoán (không log, không thông báo lỗi). Lưu ý: bài đã có cache "OK" còn hạn (`isAccessEntryFresh()` trả `true`) thì `getPostAccess()` trả thẳng từ cache, không gọi `checkPostCommentableViaTab()` nên KHÔNG bị deadlock — đây là lý do bug không lộ ra ngay lập tức với mọi lịch comment, chỉ với bài chưa từng check hoặc cache đã hết hạn (>20 phút với `pending`, xem `PENDING_ACCESS_TTL_MS`).

Fix: thêm cờ đếm `_insideQueuedTask` (`background.js`, khai báo cạnh `_taskQueue`) — tăng lên khi `enqueueTask()`'s `run()` bắt đầu chạy `taskFn()`, giảm lại trong `finally`. `checkPostCommentableViaTab()` đọc cờ này: nếu `_insideQueuedTask > 0` (đang chạy TRONG 1 task của hàng đợi rồi) thì chạy thẳng logic mở tab/đọc DOM/đóng tab (hàm `run` được tách riêng khỏi việc gọi `enqueueTask`), KHÔNG enqueue lại — vì đang ở trong hàng đợi rồi thì chắc chắn không có task nào khác đang giữ tab song song (bản chất hàng đợi tuần tự, 1 task/thời điểm), không cần khoá thêm lần nữa. Chỉ thật sự gọi `enqueueTask()` khi hàm được gọi từ NGOÀI hàng đợi — đúng trường hợp cron `warmPostAccessCache()` (bắn trực tiếp từ `chrome.alarms.onAlarm`, không nằm trong task nào của `_taskQueue`), nơi việc khoá vẫn cần thiết để không đá tab với 1 job đăng bài/comment tay đang chạy song song.

Root cause hiện tượng (2): `runScheduledJob()` trước đây hardcode CHUNG 1 tiêu đề/message thông báo ("GroupFlow — Đăng theo lịch" / "Bắt đầu đăng bài…") cho MỌI `data.kind` — biến `snippet` chỉ tính riêng cho `kind === 'post'` (lấy `noi_dung` của bài), còn `kind === 'comment'`/`'generate_image'` đều rơi vào chuỗi mặc định giống hệt post. Fix: tra bảng `scheduleNotice` theo đúng `data.kind` — comment dùng tiêu đề "GroupFlow — Comment theo lịch" + snippet lấy từ `data.payload?.comment`, generate_image dùng "GroupFlow — Tạo ảnh theo lịch".

Cả 2 fix đều nằm trong `background.js` (không đụng `fbCommentBg.js`) nên **không cần rebuild `modules/swBundle.js`**.

**GroupFlow v1.0.259 — Tab check mở liên tục không nghỉ + bỏ nút đánh dấu tay (2026-07-11):** Tony báo ngay sau khi lên v1.0.258 (check bằng tab thật): thấy tab check mở liên tục không dừng. Root cause: `warmPostAccessCache()` chưa từng có gì chặn 2 lượt gọi CHỒNG LÊN NHAU — cron `gf_check_post_access` (mỗi 3 phút) và message `GF_WARM_POST_ACCESS` (bắn mỗi lần mở/chuyển vào tab Comment) đều có thể gọi hàm này, mỗi lượt tự đọc `cache`/tính `stale` riêng mà không biết lượt kia đang xử lý dở — nếu backlog nhiều bài (Tony có ~15-20 bài chờ check cùng lúc) và lượt trước CHƯA KỊP ghi kết quả xong (giờ mỗi check tốn vài giây do phải mở/tải/đóng tab thật thay vì tức thì như fetch() cũ), lượt sau vẫn thấy y hệt tập "còn stale" rồi tự xếp thêm việc check qua `enqueueTask()` — hàng đợi dồn cục, tab mở nối đuôi liên tục không nghỉ. Lỗ hổng này **có sẵn từ trước** (không phải do v1.0.258 tạo ra) nhưng hoàn toàn VÔ HÌNH khi check còn là `fetch()` ngầm, tức thì — giờ mỗi check là 1 tab thật hiện ra vài giây nên lộ rõ ngay.

Fix: thêm cờ `_warmPostAccessInFlight` (`background.js`, pattern giống hệt `tidienSyncInFlight` đã dùng cho `syncFromTidien()`) — `warmPostAccessCache()` đổi thành wrapper mỏng kiểm tra cờ này trước khi gọi `_warmPostAccessCacheImpl()` (đổi tên hàm logic gốc); lượt gọi nào tới khi cờ đã bật (đang có lượt khác chạy) thì bỏ qua êm ngay, không xếp thêm việc — đảm bảo tại 1 thời điểm chỉ đúng 1 lượt batch đang xử lý. Đồng thời giảm batch "mở tab Comment" (`GF_WARM_POST_ACCESS` handler) từ 6 xuống 3 bài/lượt — 6 tab liên tiếp (mỗi cái vài giây) nhấp nháy nhiều hơn cần thiết giờ chúng không còn vô hình nữa.

Nhân tiện, Tony chốt bỏ hẳn nút "🚫 Đánh dấu chờ duyệt" tay (thêm ở v1.0.257, lúc chưa có cách tự động đáng tin) — vì check tự động bằng tab thật (v1.0.258) giờ đã đáng tin, không cần lối thoát thủ công nữa. Xoá `markPostPendingManually()` + `manualPendingToggleHtml()` (`sidepanel.js`) và điều kiện `entry.kind === 'manual_pending'` trong `isAccessEntryFresh()` (`fbCommentBg.js`) — dọn sạch, không để lại code chết.

**GroupFlow v1.0.258 — Check "bài còn comment được không" bằng tab thật, đảo ngược kết luận v1.0.257 (2026-07-11):** Tony tự tay làm "View Page Source" trên đúng bài đang chờ duyệt (`4450720991842547`) và tìm bằng Ctrl+F — kết quả: chữ **"Bài viết đang chờ phê duyệt"** / **"Bài viết của bạn đang chờ quản trị viên phê duyệt..."** **CÓ THẬT** trong response HTML. Điều này đảo ngược hoàn toàn kết luận v1.0.257 ("banner do JS client-side dựng sau khi tải trang, HTML thô không có") — banner đó CÓ trong HTML ngay từ đầu, không cần JS chạy thêm gì cả.

Vậy tại sao `fetch()` (dùng bởi `checkPostCommentable()` cũ) không bao giờ thấy được nó? Nguyên nhân thật, xác định qua thảo luận: **`fetch()` gọi từ JavaScript luôn tự động bị chính trình duyệt gắn cờ `Sec-Fetch-Mode: cors`** — đây là header trình duyệt tự set dựa vào CÁCH request được khởi tạo (gọi API `fetch()` trong code), **JavaScript không có cách nào ghi đè/giả mạo được cờ này** dù có set header thủ công thế nào. Ngược lại, mở tab thật (gõ URL, bấm link, `chrome.tabs.create`) luôn có `Sec-Fetch-Mode: navigate`. Nhiều khả năng Facebook's server dùng chính tín hiệu này để quyết định có render đầy đủ 1 số "pagelet" phụ (như banner trạng thái duyệt) hay lược bớt cho request kiểu "nền" — một dạng tối ưu hiệu năng phía server, không phải cơ chế chống bot cố ý. Bằng chứng gián tiếp ủng hộ giả thuyết này: comment qua GraphQL (cũng là request nền, cũng `Sec-Fetch-Mode: cors`) vẫn hoạt động bình thường — vì GraphQL là API HÀNH ĐỘNG được thiết kế sẵn để gọi bằng code (chính website Facebook cũng dùng GraphQL nền cho mọi tương tác), khác hẳn trang HTML hiển thị (được tối ưu/lược bớt tuỳ ngữ cảnh request).

Vì `Sec-Fetch-Mode` không thể giả mạo từ `fetch()`, **không có cách nào vá tiếp bằng string-matching hay đổi header** — chỉv còn 2 hướng: (1) tìm 1 GraphQL query riêng trả về đúng trạng thái duyệt dạng dữ liệu (nhẹ, nhưng cần Tony tự dò Network tab tìm đúng `doc_id`, để làm sau), hoặc (2) mở tab thật để có đúng tín hiệu "navigate". Tony chốt làm (2) trước.

Thêm `GF_BG.checkPostCommentableViaTab({ groupId, postId, session, isTimeline })` (`background.js`, đặt cạnh các hàm quản lý tab khác của Cổ điển) — mở tab NỀN (`chrome.tabs.create({ active: false, windowId })`, không cướp focus người dùng), tái dùng đúng hạ tầng đã có sẵn (`getNormalWindowId()` tìm cửa sổ Chrome thường, `waitForTabLoad()` đợi tải xong), chờ thêm 1.5 giây cho phần UI phụ kịp render (`'complete'` chỉ đảm bảo tải tài nguyên xong, không đảm bảo mọi UI đã dựng), đọc `document.body.innerText` thật qua `chrome.scripting.executeScript` (không phải HTML thô — đã qua JS render đầy đủ, đáng tin như người dùng thật nhìn thấy), rồi đóng tab lại (`chrome.tabs.remove`). Toàn bộ bọc trong `enqueueTask()` — hàng đợi tuần tự DÙNG CHUNG với Cổ điển — để không mở 2 tab đá nhau nếu đang có tác vụ đăng bài/comment thật chạy song song lúc cron quét.

`getPostAccess()` (`fbCommentBg.js`) đổi sang gọi `GF_BG.checkPostCommentableViaTab(...)` thay vì `this.checkPostCommentable(...)` (bản `fetch()` cũ) — **giữ nguyên `checkPostCommentable()` cũ trong file, không xoá**, dùng để debug/so sánh qua console nếu cần sau này. Lưu ý kỹ thuật: `GF_BG` (khai báo `const` trong `background.js`) tham chiếu được trực tiếp từ `fbCommentBg.js` (nạp qua `importScripts('modules/swBundle.js')`) mà không cần `globalThis.GF_BG` — vì service worker này là **classic script** (không có `"type":"module"` trong manifest.json), nên `importScripts()` chạy code import vào CHUNG 1 global scope với `background.js`, khiến `const`/`let` khai báo ở top-level của `background.js` vẫn được các hàm gọi SAU đó (dù định nghĩa ở file khác qua import) nhìn thấy như 1 biến thường — miễn là truy cập lúc GỌI HÀM (runtime), không phải lúc định nghĩa hàm.

Đánh đổi chấp nhận: chậm hơn hẳn `fetch()` cũ (mở/tải/đóng 1 tab thật mỗi lần check, ước ~3-5 giây/bài thay vì gần như tức thì) — nhưng đây là cách DUY NHẤT hiện tại nhận đủ tín hiệu Facebook thật sự trả về. Tìm 1 GraphQL query nhẹ hơn để thay thế lâu dài để lại làm sau.

**GroupFlow v1.0.257 — Nút đánh dấu tay "chờ duyệt" cho bài của mình, kết luận dứt điểm về giới hạn detect tự động (2026-07-11):** Tony chạy đúng lệnh force-check console (đưa ở v1.0.254/256) cho post `4450720991842547` (nhóm `2885408438373818`) — log JSON trả về `hintSnippets` với nội dung xung quanh 3 từ khoá `approv`/`pending`/`review` tìm thấy trong HTML thô. Soi kỹ: `approv` khớp trong 1 đoạn liệt kê enum cuộc gọi WebRTC (`"ADD_PARTICIPANTS","SUBSCRIPTION",...,"RING","DISMISS","CONFERENCE_STATE"`), `pending` khớp trong cấu hình livestream/video (`"multi_segment_decay":0,...,"live_stre`), `review` khớp trong tên component React nội bộ (`...ActionBarMoreMenu.react...MWChatDeleteConv`) — **cả 3 đều là trùng hợp ngẫu nhiên trong ~900KB bundle JS nội bộ của Facebook, hoàn toàn không liên quan gì tới duyệt bài**. Quan trọng nhất: 2 từ khoá tiếng Việt thật (`duyệt`, `chờ`) **không hề xuất hiện** trong `looseHintsOnOk` — nghĩa là chữ "đang chờ phê duyệt" **không tồn tại** ở bất kỳ đâu trong HTML thô server trả về.

Kết luận dứt điểm (chấm dứt chuỗi 6 lần vá marker từ v1.0.219): banner "chờ phê duyệt" chắc chắn do JavaScript phía client dựng ra SAU KHI trang tải xong (hydration) — `checkPostCommentable()` chỉ `fetch()` HTML thô, không chạy JS, nên **không có cách nào tự động phát hiện được** trạng thái này cho loại post/trang này. Đây là giới hạn cấu trúc thật sự, không phải bug có thể vá bằng cách thêm marker mới.

Fix duy nhất còn khả thi: để CHÍNH CHỦ BÀI tự đánh dấu tay khi nhìn thấy banner đó bằng mắt thường trên Facebook. Thêm:
- `markPostPendingManually(groupId, postId, pendingApproval)` (`sidepanel.js`) — gọi lại đúng `POST /api/user-sync/posts` có sẵn (không thêm endpoint mới) để báo `pending_approval` lên server (ảnh hưởng luôn tới việc bài có hiện cho "Đồng đội" hay không), đồng thời cập nhật ngay `gf_post_access_cache` cục bộ để có hiệu lực tức thì trên UI, không cần đợi đồng bộ.
- Nút "🚫 Đánh dấu chờ duyệt" / "✅ Đã duyệt xong" (`manualPendingToggleHtml()`, hiện trên card "Của tôi", cạnh nút "▶ Chạy"/"Mở bài") — chỉ áp cho nhóm đầu tiên của bài (`validGroups[0]`, đúng nhóm nút "Mở bài" trỏ tới; 1 bài đăng nhiều nhóm thì mỗi nhóm có trạng thái duyệt riêng nhưng không mở rộng UI cho ca hiếm này).
- Cache ghi bằng kind riêng `'manual_pending'` (không phải `'pending'` thường) — lý do: `'pending'` chỉ bền 20 phút (`PENDING_ACCESS_TTL_MS`), nếu dùng lại kind đó thì cron `warmPostAccessCache()` sẽ tự check lại sau 20 phút và ghi đè về `'ok'` sai y hệt cũ (vì bug detect gốc vẫn còn nguyên), làm đánh dấu tay trở nên vô nghĩa. `'manual_pending'` được cả `isPostAccessFresh()` (sidepanel.js) lẫn `isAccessEntryFresh()` (`fbCommentBg.js`, service worker — sửa thêm điều kiện mới, y hệt cách `'deleted'` đã được coi bền vĩnh viễn) coi là KHÔNG BAO GIỜ hết hạn — chỉ mất khi chính user bấm "✅ Đã duyệt xong" (ghi lại `kind: 'ok'`).
- Tag hiển thị: `commentAccessTagHtml()` phân biệt "🚫 Chờ duyệt (tự đánh dấu)" (kind `manual_pending`) với "⏳ Chờ duyệt" (tự động check ra `pending` thật) — dễ nhận biết nguồn gốc trạng thái.

Hạn chế đã biết: bài bị đánh dấu tay sẽ biến mất khỏi danh sách "Của tôi" (đúng behavior của mọi bài không actionable, qua `isCommentActionable()`) — hiện chưa có filter riêng để dễ tìm lại các bài đã tự đánh dấu (nhằm bấm "Đã duyệt xong" khi Facebook duyệt xong thật). Cần bổ sung nếu Tony thấy bất tiện khi dùng thật.

**Backend + GroupFlow v1.0.253 — Gieo lại cache "có thể comment" từ server thay vì check lại từ đầu (2026-07-11):** Tony hỏi đúng trọng tâm sau v1.0.252: "flow hiện tại là A có 100 bài, check 50 bài OK thì lấy hiển thị 50 bài của tôi — đơn giản vậy thôi. Ví dụ tao check 50 bài OK lên server rồi, giờ tải lại extension mày đi check 100 bài lại từ đầu?" — xác nhận ĐÚNG LÀ VẬY trước bản này, và đây là thiết kế chưa thông minh: cache cục bộ (`gf_post_access_cache`, quyết định "của tôi" hiện được không) và việc CHÍNH MÁY này (hay máy khác của cùng user) đã từng báo `pending_approval`/`pending_checked_at` lên server qua `reportOwnPendingApproval()` là **2 thứ hoàn toàn tách biệt, không liên kết với nhau** — mất cache cục bộ (reload, cài lại, hay bump `POST_ACCESS_CACHE_SCHEMA` như v1.0.252 vừa làm) đồng nghĩa phải fetch lại Facebook cho TỪNG bài một từ đầu, dù server đã biết câu trả lời đúng từ trước.

Fix: `GET /api/user-sync/my-posts` (`userSync.js`, cả 2 nhánh query — có `since` lẫn cold-start) trả thêm 2 cột `pending_approval`, `pending_checked_at`. Thêm `seedPostAccessCacheFromServerRows()` (`sidepanel.js`), gọi ngay trong `pullMyPostsFromServer()` sau mỗi lần đồng bộ `/my-posts` về — với mỗi bài server trả có `pending_checked_at` (tức ĐÃ có ai đó, máy nào đó, check và báo cáo rồi), "gieo" thẳng kết quả đó vào `gf_post_access_cache` cục bộ (so mốc `checkedAt` — KHÔNG ghi đè nếu cache cục bộ đang có sẵn kết quả MỚI HƠN, ví dụ vừa force-check tay ngay trước khi sync chạy). Từ giờ: reload/cài lại extension chỉ khiến những bài **CHƯA TỪNG có xác nhận nào từ server** (`pending_checked_at IS NULL` — nghĩa là chưa có máy nào từng check được) mới cần cron `warmPostAccessCache()` tự check lại từ đầu; bài đã có xác nhận (dù ở máy khác hay lần chạy trước) được khôi phục lại ngay, không phí công fetch Facebook thêm lần nào nữa. Ghi chú kỹ thuật: giá trị schema `3` được chép nguyên văn (không import được) vì `sidepanel.js` không có quyền truy cập hằng số `POST_ACCESS_CACHE_SCHEMA` (chỉ tồn tại trong `fbCommentBg.js`, chỉ bundle cho service worker) — nhớ đổi theo nếu `fbCommentBg.js` bump schema lần nữa.

**v1.0.252 — Tự xoá sạch cache "có thể comment" cũ khi lên bản mới (2026-07-11):** Tony hỏi thẳng "sao không chạy lệnh reset DB đi mà phải khổ vậy" sau khi thấy quy trình force-check qua console ở v1.0.251 rườm rà. Làm rõ: cache `gf_post_access_cache` (nguồn gây bài "của tôi" hiện sai "có thể comment") **không nằm ở DB server** — nó nằm ngay trong `chrome.storage.local` của MÁY đang chạy extension, hoàn toàn cục bộ, không đồng bộ lên đâu cả. Không có "lệnh DB" nào chạm được vào nó — chỉ có thể xoá trên đúng máy đó. Bump `POST_ACCESS_CACHE_SCHEMA` (`fbCommentBg.js`, 2→3 — lần thứ 2 dùng cơ chế này, lần đầu ở v1.0.222) — `readPostAccessCache()` phát hiện schema không khớp sẽ tự xoá sạch toàn bộ cache cũ ngay lần đọc đầu tiên sau khi reload extension lên bản này, không cần user tự gọi `force:true` qua console cho từng bài một. Đánh đổi: xoá luôn cache của MỌI bài (không chỉ đúng 1 bài đang lỗi) — chấp nhận được vì đây chỉ là cache hiệu năng (tránh check lại Facebook quá thường xuyên), không phải dữ liệu thật; cron `warmPostAccessCache()` sẽ tự check lại dần (2 bài/~3 phút, hoặc 6 bài ngay khi mở tab Comment).

**v1.0.251 — Bài "của tôi" chưa duyệt vẫn hiện "có thể comment", thêm log chẩn đoán thay vì vá mù (2026-07-11):** Tony gửi link bài thật (nhóm `2885408438373818`, post `4450720991842547`) chưa được quản trị viên nhóm duyệt nhưng tab Comment → **Của tôi** (khác hẳn bug cross-posts đã fix ở v1.0.246 — đây là bài CỦA CHÍNH MÌNH, đi qua `checkPostCommentable()` chứ không phải server `/cross-posts`) vẫn hiện "✓ Có thể comment", hỏi thẳng "sao bị hoài vậy" — đúng, đây là bug đã tái diễn 5 lần (v1.0.219 → 220 → 222 → 229 → 232), mỗi lần đoán mù 1 marker tiếng Việt mới rồi tưởng đã xong.

Kiểm tra lại giả thuyết cũ nhất (lỗi chuẩn hoá Unicode NFC/NFD, đã "vá" ở v1.0.229): xác nhận bằng script Node so từng marker hiện tại (`đang chờ phê duyệt`, `đang chờ duyệt`, `chờ quản trị viên nhóm phê duyệt`, `Bạn hiện không xem được nội dung này`...) — **tất cả đều đúng chuẩn NFC**, khớp với `.normalize('NFC')` áp lên HTML fetch về. Không phải lỗi cũ tái phát.

Giả thuyết mới, nhiều khả năng đúng hơn: `checkPostCommentable()` dùng `fetch()` lấy HTML thô — **không chạy JavaScript**. Facebook là SPA hiện đại, banner "chờ phê duyệt" (Tony từng gửi ảnh DOM thật của banner này) rất có thể được **JS phía client dựng ra sau khi trang tải xong** (hydration), KHÔNG có sẵn trong HTML server trả về ban đầu — khác hẳn "sai chữ marker" (lỗi có thể vá bằng cách thêm chuỗi mới), đây là **giới hạn cấu trúc** của cách tiếp cận string-match trên raw HTML — không marker tiếng Việt nào thêm vào sẽ cứu được nếu tín hiệu đó chưa từng tồn tại trong response.

Vì không có quyền truy cập session Facebook thật của Tony để tự fetch kiểm chứng, thay vì đoán marker thứ 6 (đã chứng minh không hiệu quả 5 lần trước), đợt này chỉ thêm **log chẩn đoán đầy đủ** (`modules/fbCommentBg.js`, `checkPostCommentable()`):
- Nhánh khớp marker OK rõ ràng (`story_title`/`story_token`/`likeAction`): trước đây KHÔNG log gì cả — thêm `console.info` kèm cờ marker nào khớp + quét thêm từ khoá rộng (`duyệt`, `approv`, `pending`, `chờ`, `review`, không phân biệt hoa/thường) — nếu các từ khoá này CÓ mặt cùng lúc với marker OK, đó là bằng chứng trực tiếp "tín hiệu chờ duyệt có tồn tại nhưng dùng từ khác với 3 marker đang dò" (khác giả thuyết SPA hydration).
- Nhánh fail-open (không khớp gì): giữ nguyên log cũ (v1.0.229, đầu 400 ký tự HTML) + thêm quét từ khoá rộng tương tự.

Rebuild `modules/swBundle.js` bắt buộc sau khi sửa (`fbCommentBg.js` nằm trong danh sách file được `build-sw-bundle.js` gộp vào service worker — sửa module gốc mà quên rebuild bundle thì service worker vẫn chạy code CŨ). Bước tiếp theo cần Tony: chờ cron `warmPostAccessCache()` (mỗi ~3 phút) check lại đúng post này → mở `chrome://extensions` → GroupFlow → "service worker" → Console → tìm log ứng với post `4450720991842547` → gửi lại nguyên văn để sửa có bằng chứng, thay vì đoán tiếp.

**Backend + Frontend — Xoá tính năng trùng lặp "admin tự lấy license key" + GroupFlow v1.0.250 hiện version trong UI (2026-07-11):** Trong lúc xử lý vụ giới hạn thiết bị, đã thêm nhầm `GET /api/user-auth/admin/my-key` (`userAuth.js`) + khu "License key GroupFlow của tôi" (`UserManagement.jsx`) mà KHÔNG tra cứu code trước — phát hiện trễ: tính năng này **đã tồn tại sẵn từ trước**, ở `GET/POST /api/auth/my-license` (`routes/auth.js`, comment gốc: "License key để dùng extension GroupFlow dưới đúng tài khoản đang đăng nhập — mọi role") + UI "License key của tôi" trong `GroupExtensionSettings.jsx` (mount ở trang Cài đặt, `Settings.jsx`) — admin/super_admin đã tự lấy được key ở đó từ trước, không hề bị thiếu tính năng như tưởng. 2 route trùng nhau cùng ghi `license_keys` cho cùng 1 `user_id` nhưng default `plan` KHÁC NHAU (`/my-license` → free, `/admin/my-key` → enterprise) — route nào chạy TRƯỚC quyết định plan thật (route sau chỉ đọc lại row đã có, không tạo mới) — rủi ro dữ liệu thật, nghi ngờ là 1 phần nguyên nhân lỗi "Key đã hết hạn" Tony gặp khi test (dù chưa xác nhận được 100% — cả 2 route đều insert `expires_at: null`, không nơi nào trong code hiện tại set expiry về quá khứ, nên nguồn gốc thật của `expires_at` đã set có thể là dữ liệu cũ hơn, cần soi DB trực tiếp mới chắc). Xoá bỏ hẳn phần trùng lặp mới thêm, giữ nguyên `/api/auth/my-license` + `GroupExtensionSettings.jsx` sẵn có làm đường DUY NHẤT cho "tự lấy key riêng".

Đồng thời thêm theo yêu cầu Tony: `renderExtensionVersion()` (`sidepanel.js`, đọc `chrome.runtime.getManifest().version`) — hiện số version ngay cạnh "GroupFlow" ở header, không phụ thuộc đã kích hoạt hay chưa (header nằm trong DOM dù overlay kích hoạt che phía trên) — biết ngay đang chạy đúng bản mới nhất sau khi reload extension, không cần mở `chrome://extensions` để soi.

**v1.0.249 + Backend — Đã kích hoạt rồi vẫn tự bật lại màn hình kích hoạt khi chuyển tab (2026-07-11):** Tony báo: đã active key xong xuôi, chỉ chuyển qua tab khác trong extension lại bị bắt kích hoạt lại từ đầu — bug thật, không phải hiểu nhầm hành vi. Root cause nằm ngay trong tính năng mới thêm cùng ngày (v1.0.245, `recheckLicenseStillValid()` — gọi lại `validate-key` mỗi giờ qua chu kỳ nền `gf_tidien_sync` để phát hiện key bị admin khoá/hết hạn): hàm này lấy NGUYÊN response trả về ghi đè vào `licenseInfo` cache mà không kiểm tra response đó có thật sự là "kết quả check license" hay không. Nếu request đó vô tình bị `licenseValidateLimiter` chặn (HTTP 429 `"Thử lại quá nhiều lần"` — rất dễ dính khi đang test, vì 1 lượt "Đặt lại thiết bị" (v1.0.247/248) tự tốn 2 request liên tiếp qua CÙNG limiter này), response 429 không có field `valid:true` — bị ghi đè thẳng vào `licenseInfo`, làm mất cờ `valid` đang cache đúng. `checkLicenseGate()` (`sidepanel.js`) đọc lại thấy `licenseInfo.valid` không còn `true` → coi là "chưa hợp lệ" → tự bật lại `#gf-activation-overlay`, dù key thực ra vẫn đang hoàn toàn hợp lệ, chỉ là request kiểm tra định kỳ bị rate-limit tạm thời.

Fix 2 phần: (1) `recheckLicenseStillValid()` giờ chỉ tin (và ghi đè cache) khi HTTP `res.ok` (200) VÀ `data.valid` là kiểu `boolean` thật — mọi trường hợp khác (lỗi HTTP bất kỳ, rate-limit, response dạng lạ/thiếu field) đều fail-open, không đụng gì tới `licenseInfo` đã cache trước đó — tách bạch rõ "lỗi khi GỌI request" khỏi "kết quả THẬT về tình trạng key". (2) Nâng `licenseValidateLimiter` (`rateLimit.js`) từ 10 lên 30 request/15 phút/IP — giới hạn 10 quá chặt cho nhu cầu dùng thật (không phải phòng brute-force key thật — key là UUID 32 ký tự hex, ~10^38 khả năng, không rate limit nào cản được kiểu dò đó cả; limiter này chỉ nhắm chặn spam/DoS), đặc biệt dễ dính khi 1 hành động UI (Đặt lại thiết bị) tự nhân đôi số request qua cùng 1 limiter.

**v1.0.248 — Nút "Đặt lại thiết bị" đặt sai chỗ, không bấm tới được đúng lúc cần nhất (2026-07-11):** Tony truy lại đúng kịch bản thật: "user A xài laptop A, xoá extension cài lại bản mới, active key cũ thì chuyện gì xảy ra?" — lần theo đúng luồng: xoá extension xoá sạch `chrome.storage.local` (mất `gfDeviceId` cũ D1) → cài lại → kích hoạt sinh `device_id` MỚI D2 (không liên quan D1) → nếu D1 vẫn còn "hoạt động" theo `DEVICE_STALE_DAYS` (chưa đủ 14 ngày từ lần dùng cuối) → **vẫn bị chặn y hệt trước v1.0.247** (cơ chế tự hết hạn thêm ở bản đó KHÔNG cứu được trường hợp cài lại NGAY, chỉ cứu nếu để lâu >14 ngày). Lúc này user cần bấm nút "🔄 Đặt lại thiết bị" — nhưng phát hiện lỗi thật: nút đó (v1.0.247) đặt trong tab Cài đặt → Đồng bộ, mà tab Cài đặt nằm SAU màn hình kích hoạt `#gf-activation-overlay` (`position:fixed;inset:0;z-index:9999`) — overlay che kín TOÀN BỘ, chặn hết mọi thao tác phía sau nó. Đúng lúc user cần nút reset nhất (bị chặn ngay từ lần kích hoạt đầu) thì KHÔNG THỂ bấm tới được nút đó — tự vô hiệu hoá chính giải pháp mới làm.

Fix: thêm `#overlayResetDevicesBtn` ngay trên overlay kích hoạt (`sidepanel.html`, ẩn mặc định bằng class `hidden`) — chỉ hiện khi response `validate-key` trả `code: 'device_limit_reached'` (`checkLicenseGate()`, `sidepanel.js`). Refactor logic gỡ-thiết-bị-rồi-kích-hoạt-lại thành hàm dùng chung `resetMyDevicesAndReactivate(key)` — cả nút overlay (kịch bản bị chặn ngay từ đầu) lẫn nút trong Cài đặt (kịch bản đã có phiên hợp lệ, chủ động dọn trước) đều gọi chung 1 hàm, tránh trùng code. Style riêng `.gf-activation-reset` (viền, nhạt hơn nút "Xác thực key" chính — hành động ít gặp hơn, không nên lấn át nút chính).

**v1.0.247 — Thiết bị tự hết hạn sau 14 ngày + tự đặt lại thiết bị + admin tự lấy key riêng (2026-07-11):** Tiếp nối vụ "Key gói free chỉ dùng được tối đa 1 thiết bị" — Tony hỏi liên tiếp: (1) vì sao admin/super_admin không có trong "GroupFlow Users" để tự sửa, (2) tạo key mới có ảnh hưởng bài đăng cũ không, (3) chỉ ra vấn đề THIẾT KẾ gốc: `device_id` (`modules/tidienAuth.js`, `getDeviceId()`) chỉ sống trong `chrome.storage.local` — Chrome extension không có quyền đọc bất kỳ thông tin phần cứng thật nào để tự nhận ra "vẫn là máy này" — nên cài lại extension/xoá dữ liệu duyệt web sinh `device_id` MỚI, hoàn toàn không liên quan tới thiết bị cũ. Trước bản này, thiết bị cũ (dù đã bỏ hoang) chiếm slot **vĩnh viễn** trong `license_key_devices` cho tới khi admin chủ động gỡ qua UI — một user chỉ xài đúng 1 máy vẫn có thể tự nhiên bị khoá sau 1 lần cài lại, đây là lỗ hổng thiết kế thật chứ không phải hiểu nhầm.

3 phần độc lập, giải quyết 3 khía cạnh khác nhau:

1. **Tự hết hạn (fix gốc, quan trọng nhất)**: `registerOrCheckDevice()` (`licenseDeviceService.js`) đổi từ đếm TỔNG mọi thiết bị từng đăng ký sang chỉ đếm thiết bị CÒN HOẠT ĐỘNG — `last_seen_at` trong `DEVICE_STALE_DAYS = 14` ngày gần nhất. Thiết bị bỏ hoang quá hạn không còn tính vào giới hạn plan nữa, và tự bị `DELETE` ngay khi có thiết bị mới cần chỗ (không đợi admin/cron dọn riêng). Đánh đổi: về lý thuyết 1 key có thể "luân phiên" nhiều hơn N máy nếu mỗi máy dùng thưa hơn 14 ngày — chấp nhận được vì mục tiêu là chặn NHIỀU máy dùng ĐỒNG THỜI (chia sẻ key), không phải đếm tuyệt đối lịch sử. `countDevicesForKey()` (dùng cho cột "Thiết bị" ở `GET /admin/users`) đổi theo cùng công thức để số hiển thị khớp đúng số thật đang enforce. `listDevicesForKey()` trả thêm cờ `stale` (tính toán trong SQL) — `UserManagement.jsx` (tab chi tiết user → "Thiết bị") hiện thiết bị stale mờ đi kèm tag "⏳ hết hạn (không tính vào giới hạn)".
2. **Tự đặt lại thiết bị — escape hatch ngay lập tức, không đợi 14 ngày**: `POST /api/user-auth/reset-devices` (route mới, `licenseValidateLimiter` như validate-key, public) — nhận `{ key, deviceId, deviceLabel }`, gỡ TOÀN BỘ thiết bị đang đăng ký cho đúng key đó rồi đăng ký lại thiết bị hiện tại. Quan trọng: route này **KHÔNG đổi `key_value`** và **KHÔNG đụng `user_posts`** — bài đăng cũ khoá theo `user_account_id`, hoàn toàn độc lập với `license_key_devices`/`key_value`, nên reset thiết bị (hay kể cả đổi hẳn key) không bao giờ ảnh hưởng lịch sử bài đã đăng. Chặn nếu `key.status !== 'active'` (key bị admin suspend/hết hạn) — không cho tự lách suspend bằng cách "reset thiết bị". Extension: nút "🔄 Đặt lại thiết bị" (`sidepanel.html`, Cài đặt → Đồng bộ, cạnh nút "Thoát") — gọi `reset-devices` rồi tự gọi lại `validate-key` ngay để kích hoạt lại máy hiện tại, không cần thoát/nhập tay lại key.
3. **Admin/super_admin tự lấy license key riêng**: xác nhận với Tony đây là 2 hệ thống tài khoản tách biệt hoàn toàn — "GroupFlow Users" (`GET /admin/users`) lọc cứng `role = 'group_user'`, license key chỉ được tạo qua `POST /register` (form đăng ký công khai) — admin/super_admin (đăng nhập website bằng JWT riêng, `middleware/auth.js`) trước đây không có đường nào lấy key. Thêm `GET /api/user-auth/admin/my-key` (yêu cầu JWT admin thường) — tự tạo license key (plan `enterprise`) cho `req.user.id` nếu chưa có, trả nguyên key cũ nếu đã có (không có "regenerate" ở v1). `UserManagement.jsx` thêm khu `MyGroupFlowKey` (hiện trên cả 2 tab của trang) — nút "Lấy / Tạo key" + hiện key kèm copy.

**v1.0.246 — Bài chờ duyệt vẫn hiện "✓ Có thể comment" trong list Comment đồng đội (2026-07-11):** Tony báo dù v1.0.236 đã đảo hướng cross-posts sang opt-in (server chỉ trả bài đã CONFIRM OK), bài chờ duyệt vẫn còn hiện trong list Comment tab Đồng đội. Không phải backend sai — `GET /cross-posts` (`userSync.js`) đã lọc đúng `pending_approval=0 AND pending_checked_at còn hạn`. Root cause nằm ở EXTENSION: `crossPostsCache` (`chrome.storage.local`) được nạp bằng kiểu **merge cộng dồn** (`mergeUserPostsById()` ở `sidepanel.js`, và logic Map tương tự trong `runFlow1BackgroundSync()` ở `background.js`) — sync dùng cursor `since` (incremental), chỉ THÊM/CẬP NHẬT bài có trong response mới nhất, KHÔNG BAO GIỜ xoá bớt bài đã cache từ trước. Nếu 1 bài từng được server xác nhận OK (nằm trong cache), sau đó chủ bài mở lại extension và `warmPostAccessCache()` phát hiện nó thật ra đang chờ duyệt (báo `pending_approval=1` lên server) → server ngừng trả bài này trong các lần fetch tiếp theo — nhưng bản cache CŨ trên máy đồng đội vẫn còn nguyên, không ai xoá nó đi. Cộng thêm `isCommentActionable()` (`sidepanel.js`) cho bài `_source: 'cross'` code cứng `return true` — không có bất kỳ kiểm tra hạn nào — nên cache cũ này được tin là actionable vĩnh viễn, hiện "✓ Có thể comment" sai.

Fix: `GET /cross-posts` trả thêm cột `up.pending_checked_at` (mốc chủ bài xác nhận gần nhất). Extension thêm `CROSS_POST_CONFIRMED_TTL_MS` (6 giờ — mirror đúng `OK_CONFIRMED_TTL_MS` phía server) — `isCommentActionable()` giờ chỉ coi bài đồng đội là actionable nếu `pending_checked_at` của CHÍNH bài đó còn trong hạn 6 giờ, không còn tin theo "có mặt trong cache" nữa. Vì đây là field HOÀN TOÀN MỚI, mọi entry cache có từ TRƯỚC bản này (chưa từng có `pending_checked_at`) sẽ tự bị coi là hết hạn ngay sau khi cập nhật extension — hiệu ứng phụ có lợi: tự dọn sạch mọi bài đang "kẹt" hiện sai, chỉ hiện lại nếu server xác nhận lại trong lần sync kế tiếp.

**v1.0.245 — Tự phát hiện key bị khoá/hết hạn qua chu kỳ nền (2026-07-11):** Tony hỏi nếu admin suspend hoặc key hết hạn thì extension đang chạy sẵn có tự biết/tự đăng xuất không. Trước bản này KHÔNG: `checkLicenseGate()` (`sidepanel.js`) chỉ đọc lại `licenseInfo.valid` cache từ lần `validate-key` cuối cùng lúc kích hoạt, không bao giờ tự gọi lại server — dù các route đồng bộ (`userSync.js`, qua `authenticateLicenseKey`) có check live mỗi request, chỉ làm hỏng đồng bộ ngầm chứ không đưa user quay lại màn hình kích hoạt. Tony chốt cơ chế check "từ từ, không cần ngay lập tức" là đủ — không cần kênh real-time riêng.

Thêm `GF_BG.recheckLicenseStillValid(auth)` (`background.js`) — gọi lại đúng `POST /api/user-auth/validate-key` (endpoint có sẵn từ lúc kích hoạt, không cần route mới) kèm `deviceId` đã lưu (thiết bị đã đăng ký rồi nên `registerOrCheckDevice()` chỉ update `last_seen_at`, không tốn thêm slot), ngay trong chu kỳ nền `gf_tidien_sync` sẵn có (`_syncFromTidienImpl()`, chạy dù panel có mở hay không) — không thêm alarm riêng. Tự throttle 1 giờ/lần qua `licenseRecheckAt` (tách khỏi `tidienAutoSyncMinutes`, tránh dồn request vào `licenseValidateLimiter` — 10 req/15 phút/IP — khi nhiều máy chung 1 IP mạng công ty). Nếu key bị revoke: ghi `licenseInfo.valid=false` (giữ nguyên `licenseKey` để overlay tự điền lại), không xoá gì thêm. Lần mở panel kế tiếp, `checkLicenseGate()` thấy `licenseInfo.valid !== true` nên tự hiện lại `#gf-activation-overlay` — sửa thêm để hiện SẴN đúng lý do (`licenseInfo.error`) ngay khi mở, không bắt user phải tự bấm "Xác thực key" lại mới biết vì sao bị đá về. Lỗi mạng/parse response thì fail-open (coi như vẫn hợp lệ) — không tự đăng xuất oan chỉ vì mất mạng tạm thời.

Lưu ý kỹ thuật: service worker (`background.js`) KHÔNG nạp `modules/tidienAuth.js` — file đó chỉ được `<script>` cho `sidepanel.html` (context có `window`), không nằm trong danh sách bundle của `build-sw-bundle.js` cho service worker. Gọi `GF.tidienAuth.getDeviceId()` trong `background.js` sẽ lỗi `GF is not defined` — hàm mới tự đọc thẳng key storage `gfDeviceId` (cùng key mà `getDeviceId()` dùng) thay vì gọi qua module đó.

**v1.0.244 — Màn hình vẫn tự tắt dù đã bật chặn sleep (2026-07-11):** Tony báo dù đã bật tính năng chặn máy ngủ (v1.0.195) thì màn hình Windows vẫn tự tắt. Không phải bug — đúng như thiết kế ban đầu: `chrome.power.requestKeepAwake('system')` (`background.js`) chỉ chặn **MÁY** vào chế độ sleep/suspend do idle, KHÔNG chặn **MÀN HÌNH** tắt/dim — Windows có 2 idle timer độc lập (display timeout thường ngắn hơn system sleep timeout), lúc v1.0.195 cố tình chọn mức `'system'` để đỡ tốn điện/khó chịu vì cho rằng JS nền không cần màn hình sáng để chạy. Thực tế Tony gặp: nhiều máy/policy khi màn hình tắt rồi bật lại yêu cầu đăng nhập lại (khoá máy) — gây gián đoạn tương tự sleep dù service worker vẫn sống, nên giờ ưu tiên màn hình không tắt hơn tiết kiệm điện. Fix: đổi sang `chrome.power.requestKeepAwake('display')` — theo tài liệu Chrome mức này bao luôn hiệu ứng chặn sleep của `'system'`, cộng thêm chặn màn hình tắt/dim. Giới hạn không đổi: vẫn không chặn được nếu user chủ động bấm Sleep/đóng nắp laptop.

**v1.0.243 — Đồng bộ lại 2 chỗ lệch mockup redesign: màu promo card + nút "Lên lịch đã chọn" (2026-07-11):** Tony gửi 2 ảnh so sánh sau khi redesign v1.0.228-235 đã áp mockup (`docs/design/GroupFlow Redesign.dc.html`) vào extension — chỉ ra 2 chỗ cùng 1 thành phần nhưng hiện khác nhau ở 2 nơi trong app.

1. **Chip quảng cáo (Zalopilot / Đặt xe về quê) không đồng nhất**: overlay kích hoạt license (`.gf-activation-overlay`) hiện đúng mockup — nền pastel nhạt, chữ màu theo brand. Nhưng footer chính `#gfPromoFooter` (luôn hiện ở mọi tab, `footer.gf-promo-footer`) lại hiện gradient đặc + chữ trắng — dù CẢ HAI cùng dùng chung class `.gf-promo-card`/`.gf-promo-card-zalo`/`.gf-promo-card-xe`. Root cause: gradient/chữ trắng là lựa chọn có chủ đích từ v1.0.227 ("Xanh Zalo — dễ nhận diện đúng brand đang quảng bá", xem comment cũ trong `sidepanel.css`) — cố tình lệch mockup để chip nổi bật hơn, nhưng nhìn không nhất quán với phần còn lại của app (và với chính overlay dùng cùng class). Fix: bỏ hẳn gradient + `color:#fff`, quay lại đúng token mockup — `.gf-promo-card-zalo` nền `oklch(95% 0.03 258)` chữ `oklch(40% 0.1 258)`, `.gf-promo-card-xe` nền `oklch(96% 0.03 55)` chữ `oklch(45% 0.1 55)`, border-radius 10px (bỏ luôn biến thể pill 999px riêng cho overlay) — giờ chỉ còn 1 style áp dụng mọi nơi.
2. **Nút "Lên lịch đã chọn" khác class giữa 2 tab**: footer tab **Tạo bài** (`#btnScheduleCampaign`) dùng `.btn.outline` (trắng, viền — đúng mockup, xem dòng 125 file mockup: `background:#fff; border:1.5px solid...`), nhưng footer tab **Comment** (`#btnScheduleComments`) cùng nhãn/cùng mục đích lại dùng `.btn.primary` (xanh đặc) — không có lý do thiết kế nào cho sự khác biệt này, chỉ là sót khi 2 footer được viết ở 2 thời điểm khác nhau. Đổi `#btnScheduleComments` sang `.btn.outline` cho khớp.
