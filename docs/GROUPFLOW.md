...
1. User A đăng + `POST /api/user-sync/posts` (license key) → `user_posts` trên website (`needs_comment=1`)
   - **Bài chờ admin duyệt tự "báo hộ" (v1.0.234 + backend)**: Facebook chỉ cho CHỦ BÀI xem banner "chờ phê duyệt" — người khác (đồng đội) mở cùng bài chỉ thấy trang khóa trắng, không dò được gì, dẫn tới thử comment vô ích vào bài chưa duyệt của người khác. `warmPostAccessCache()` (`background.js`) khi check ĐÚNG bài của chính mình (postQueue/serverMyPosts, không áp dụng cho bài đồng đội) ra kết quả `pending`/`ok` thì quá giang báo luôn qua `POST /api/user-sync/posts` (field optional `pending_approval`, không thêm endpoint/cron nào) — `user_posts.pending_approval`/`pending_checked_at` (migration 041). `GET /cross-posts` loại bài có cờ này còn trong TTL 30 phút (`PENDING_APPROVAL_TTL_MS`, `userSync.js`) — cố tình có TTL thay vì tin vĩnh viễn: nếu chủ bài không mở lại extension để cập nhật, cờ tự hết hạn, bài lại hiện bình thường, không bao giờ bị chặn vĩnh viễn chỉ vì 1 người không mở app.
