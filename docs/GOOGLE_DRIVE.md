...
- **Fix "Upload ảnh trong Post Editor lỗi khi Cài đặt = Google Drive" (2026-07-22)**: `POST /upload/image` (`backend/src/routes/upload.js`) trước đây chọn multer disk storage hay memory storage bằng `isUsingGoogleDrive() ? memoryStorage() : diskStorage()` **ngay tại module scope** — biểu thức này chỉ chạy 1 lần lúc `app.js` import route (`import uploadRoutes from './routes/upload.js'`, xếp trước `dotenv.config()` và `loadAppSettings()` trong file đó) nên luôn đọc cache settings rỗng → luôn chọn `false` (disk storage), vĩnh viễn cho tới khi restart process, bất kể sau đó admin chọn gì trong Cài đặt. Khi Cài đặt thật sự = Google Drive, route handler (đọc `isUsingGoogleDrive()` đúng vì gọi lại mỗi request) tưởng nhầm đang ở chế độ Drive, cố đọc `req.file.buffer` để đẩy lên Drive — nhưng multer thực tế ghi ra `req.file.path` (disk), `buffer` luôn `undefined` → upload thất bại, file rác nằm lại `public/images/` không dùng tới. Chế độ VPS local không bị ảnh hưởng vì trùng với hành vi mặc định lúc boot. Đã sửa: `imageUpload` luôn dùng `multer.memoryStorage()`; route handler tự quyết định ghi đĩa hay đẩy Drive NGAY trong handler, nơi `isUsingGoogleDrive()` luôn phản ánh đúng Cài đặt hiện tại của từng request.
