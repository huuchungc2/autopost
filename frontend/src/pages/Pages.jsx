      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between md:gap-6 mb-6">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-1 text-sm text-slate-600">
            {isSuperAdmin ? 'Quản lý tất cả fanpage — gán cho admin khi tạo mới / khi sửa.' : 'Fanpage được gán — admin có thể thêm fanpage mới.'}
          </p>
        ...
        {canManagePages && (
          ...
          <div className="flex gap-2 flex-wrap">
            <Button type="button" onClick={openCreateModal} className="bg-indigo-600 hover:bg-indigo-700">
              <Plus size={18} />
              Thêm fanpage
            </Button>
          </div>
              ...
              {pages.map((page) => (
                ...
                <tr key={page.id} className="align-top">
                  ...
                  <td className="px-4 py-3">
                    ...
                    {!page.is_active && (
                      <div className="mt-1 text-xs text-slate-500">Đang tắt</div>
                    )}
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex flex-wrap gap-2">
                      {(page.skills?.length ? page.skills : (page.skill_id ? [{ id: page.skill_id, name: getName(page.skill_id, skills) }] : [])).map((skill) => (
                        <span key={skill.id} className="inline-flex items-center rounded-full bg-indigo-50 px-3 py-1 text-xs font-semibold text-indigo-700 border border-indigo-100">
                          {skill.name}
                        </span>
                      ))}
                  ...
                  <td className="px-4 py-3">
                    <div className="flex flex-wrap gap-2">
                      <Button type="button" variant="outline" size="sm" onClick={() => handleEdit(page)}>Sửa</Button>
                      <Button type="button" variant="secondary" size="sm" onClick={() => loadTopics(page)}>Chủ đề</Button>
                      <Button type="button" variant="secondary" size="sm" onClick={() => openTokenModal(page.id)}>Token</Button>
                      {canManagePages && (
                        <Button type="button" variant="destructive" size="sm" onClick={() => handleDelete(page.id)}>Xóa</Button>
                      )}
                    </div>
                  </td>
