 ...
 *
 * Port từ prompts/fanpage-prompt.js (CommonJS) sang ESM cho backend.
 * Hiện chưa được gọi trực tiếp — flow fanpage hiện tại dùng JSON output
 * (xem contentGenerationService.js); giữ file này làm tham chiếu/định
 * hướng cho ratio 70/20/10 đã tích hợp additive vào buildGenerationSystemPrompt.
 */

export function buildFanpagePrompt({ projectContext, topic, recentPostsTypes, postType }) {
  const ratioGuide = `
