...
export async function runDueTopicSlots() {
  ...
  try {
    topics = await query(
      `SELECT ct.*, fp.id AS fb_pages_id, fp.page_id AS fb_page_id, fp.page_token, fp.name AS page_name
...
export async function generateAndPublishTopic(topic) {
  ...
  const readyPost = await ensurePostImageForPublish(postRows[0]);

  const response = await publishToFacebookWithFallback({
    internalPageId: topic.fb_pages_id,
  ...
  await persistFacebookPublishIds(postId, response, {
    hasImage: readyPost.media_type === 'image' && Boolean(readyPost.image_url),
