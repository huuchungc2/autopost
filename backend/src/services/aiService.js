...
function textEndpoint(provider, model) {
  const kind = resolveProviderKind(provider);
  if (provider?.api_endpoint) return formatEndpoint(provider.api_endpoint, model);
  return DEFAULT_TEXT_ENDPOINTS[kind] || DEFAULT_TEXT_ENDPOINTS.openai;
}

async function callOpenAI({ apiKey, model, systemPrompt, userPrompt, endpoint }) {
  const url = endpoint || DEFAULT_TEXT_ENDPOINTS.openai;
...
async function callClaude({ apiKey, model, systemPrompt, userPrompt, endpoint }) {
  const url = endpoint || DEFAULT_TEXT_ENDPOINTS.claude;
  const response = await axios.post(
    url,
    {
...
async function callGemini({ apiKey, model, systemPrompt, userPrompt, endpoint }) {
  const modelName = model || 'gemini-1.5-flash';
  const url = formatEndpoint(endpoint || DEFAULT_TEXT_ENDPOINTS.gemini, modelName);
...
export async function generateText(userPrompt, providerConfig = null, systemPrompt = '') {
  ...
  const endpoint = textEndpoint(providerConfig, model);
  ...
  return callWithRateLimit(kind, async () => {
    try {
      if (kind === 'claude') {
        return await callClaude({ apiKey: apiKey || process.env.CLAUDE_API_KEY, model, systemPrompt, userPrompt, endpoint });
      }
      if (kind === 'gemini') {
        return await callGemini({ apiKey: apiKey || process.env.GEMINI_API_KEY, model, systemPrompt, userPrompt, endpoint });
      }
      if (kind === 'openai' || providerConfig) {
        return await callOpenAI({ apiKey: apiKey || process.env.OPENAI_API_KEY, model, systemPrompt, userPrompt, endpoint });
      }
